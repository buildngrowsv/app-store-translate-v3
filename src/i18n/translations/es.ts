export const es = {
  home: {
    hero: {
      title: "Transforma tu Presencia en la App Store Globalmente",
      subtitle: "Genera descripciones atractivas, optimiza para ASO y alcanza audiencias globales con traducciones instantáneas",
      startTrial: "Comenzar Prueba Gratuita",
      login: "Iniciar Sesión",
      translateInto: "Traduce tu app a",
      aiTranslation: "Traducción Potenciada por IA",
      aiTranslationDesc: "Traduce instantáneamente el contenido de tu app a más de 50 idiomas manteniendo el contexto y el tono",
      asoOptimization: "Optimización ASO",
      asoOptimizationDesc: "Aumenta la visibilidad con palabras clave localizadas y listados optimizados",
      startTranslating: "Comienza a Traducir Ahora"
    },
    features: {
      aiTitle: "Descripciones Potenciadas por IA",
      aiDesc: "Genera descripciones atractivas que convierten usando tecnología avanzada de IA",
      asoTitle: "Optimización ASO",
      asoDesc: "Apunta a las palabras clave correctas para mejorar tu visibilidad en la tienda de aplicaciones",
      globalTitle: "Alcance Global",
      globalDesc: "Traduce el contenido de tu aplicación a más de 50 idiomas instantáneamente"
    },
    stats: {
      items: [
        { value: "50+", label: "Idiomas Soportados" },
        { value: "10K+", label: "Apps Optimizadas" },
        { value: "95%", label: "Satisfacción del Cliente" },
        { value: "2X", label: "Aumento Promedio de Descargas" }
      ]
    },
    testimonials: {
      title: "Lo Que Dicen Nuestros Clientes",
      items: [
        {
          content: "ReachMix transformó por completo nuestra presencia global. Las traducciones son precisas y naturales.",
          author: "María García",
          role: "Desarrolladora de Apps"
        },
        {
          content: "La optimización ASO realmente funciona. Nuestras descargas se duplicaron en tres meses.",
          author: "Carlos Rodríguez",
          role: "CEO de AppTech"
        },
        {
          content: "El mejor servicio de traducción que hemos usado. Rápido, preciso y con excelente soporte.",
          author: "Ana Martínez",
          role: "Gerente de Producto"
        }
      ]
    },
    comparison: {
      title: "Ve la Diferencia",
      before: {
        title: "Antes de ReachMix",
        content: "Aplicación básica de calculadora para uso diario. Te ayuda a calcular números rápidamente.\n\nCaracterísticas:\n- Suma\n- Resta\n- Multiplicación\n- División"
      },
      after: {
        title: "Después de ReachMix",
        content: "Transforma tus cálculos diarios con nuestra calculadora intuitiva y potente. Experimenta cálculos ultrarrápidos con una interfaz bellamente diseñada que hace que las matemáticas sean pan comido.\n\nCaracterísticas Principales:\n- Cálculos instantáneos con resultados en tiempo real\n- Operaciones matemáticas completas\n- Interfaz limpia y moderna para un uso sin esfuerzo\n- Perfecta para estudiantes, profesionales y todos los demás"
      }
    },
    pricing: {
      title: "Precios Simples y Transparentes",
      starter: {
        title: "Inicial",
        price: "49",
        description: "Perfecto para desarrolladores individuales",
        features: ["1 Proyecto", "Todas las Funciones", "3 Idiomas en Prueba"]
      },
      growth: {
        title: "Crecimiento",
        price: "99",
        description: "Ideal para equipos pequeños",
        features: ["3 Proyectos", "Todas las Funciones", "Soporte Prioritario"]
      },
      enterprise: {
        title: "Empresa",
        price: "Contáctanos",
        description: "Para organizaciones más grandes",
        features: ["Proyectos Ilimitados", "Soluciones Personalizadas", "Soporte Dedicado"]
      }
    },
    footer: {
      company: "ReachMix",
      tagline: "Transforma tu presencia global en la app store",
      companyLinks: "Empresa",
      about: "Acerca de",
      privacy: "Política de Privacidad",
      terms: "Términos de Uso",
      resources: "Recursos",
      blog: "Blog",
      documentation: "Documentación",
      contact: "Contacto",
      getInTouch: "Contáctanos",
      support: "Soporte",
      copyright: "© {year} ReachMix. Todos los derechos reservados."
    }
  }
};